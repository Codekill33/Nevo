...
// 1. CHECKS: Verify ledger age to prevent flash donations
        Self::verify_ledger_age(&env, contribution.last_ledger)?;

        // 2. EFFECTS: Zero the balance BEFORE the token transfer (CEI pattern)
        // This prevents re-entrancy attacks by ensuring the balance is 0 before external calls.
        ...
        // 3. INTERACTIONS: Transfer tokens back to contributor
        use soroban_sdk::token;
...
// 1. EFFECTS: Zero out the contribution record BEFORE the transfer
        // This prevents double refunds and re-entrancy.
        let zeroed_contribution = PoolContribution {
            pool_id,
            contributor: contributor.clone(),
            amount: 0,
            asset: contribution.asset.clone(),
            last_ledger: contribution.last_ledger,
        };
        env.storage()
            .instance()
            .set(&contribution_key, &zeroed_contribution);

        // 2. INTERACTIONS: Transfer tokens (External call)
        use soroban_sdk::token;
        let token_client = token::Client::new(&env, &contribution.asset);
        token_client.transfer(
            &env.current_contract_address(),
        ...
        // 3. CLEANUP: Release pool lock (Final step)
        release_pool_lock(&env, pool_id);
...
fn verify_ledger_age(env: &Env, last_ledger: u32) -> Result<(), CrowdfundingError> {
        if env.ledger().sequence() <= last_ledger {
    ...
    pub fn get_pool_contributions_paginated(
        env: Env,
        pool_id: u64,
        offset: u32,
        limit: u32,
