...
fn test_refund_fails_in_same_ledger() {
    ...
    // Advance time past deadline but keep same ledger sequence
    let grace_period = 604800u64;
    ...
    // Advance ledger sequence and try again - should succeed
...
fn test_cancel_campaign() {
    ...
    client.create_campaign(&campaign_id, &title, &creator, &10_000, &(env.ledger().timestamp() + 86400), &token_address);
    client.mock_auths(&[/* auth logic */]).cancel_campaign(&campaign_id);

    let status = client.get_campaign_status(&campaign_id);
    assert_eq!(status, crate::base::types::CampaignLifecycleStatus::Cancelled);
}

// ... include the rest of your test_donate_to_cancelled_campaign_fails and test_refund_campaign tests below ...
